...
columnTitles = thead.find_elements(
    By.XPATH, '//div[@class="cubinvest-l-table__th"][not(@style="display: none;")]')
columnTitles.pop(0)
...
for RowIdx, row in enumerate(rows):
    cells = row.find_elements(
        By.XPATH, './/div[contains(@class, "cubinvest-l-table__td")][not(@style="display: none;")][not(@style="display:none;")]')
    cells.pop(0)
    cells.pop(len(cells)-1)
    for cellIdx, cell in enumerate(cells):
        if (cell.text.__contains__("基金")):
            sheet.cell(row=RowIdx+2, column=cellIdx+1).value = cell.text[:cell.text.index(
                "基金")+2] + "\n" + cell.text[cell.text.index("基金")+2:]
        elif (cell.get_attribute("data-th") == "DjMktPrice"):
            elements = cell.find_elements(
                By.CLASS_NAME, 'cubinvest-l-table__item')
            sheet.cell(row=RowIdx+2, column=cellIdx +
                       1).value = elements[0].text + "\n" + elements[1].text
        else:
            sheet.cell(row=RowIdx+2, column=cellIdx+1).value = cell.text
            sheet.cell(row=RowIdx+2, column=cellIdx +
                       1).font = Font(color='ff0000') if cell.text.startswith("-") else Font(color='000000')
